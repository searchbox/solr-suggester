...
public class SuggeterDataStructureBuilder {
	...
	private String[] getTokens(String fulltext) {
		...
		try {
			TokenStream tokenStream = analyzer.tokenStream(fields[0],
	...
	private void iterateThroughDocuments(SolrIndexSearcher searcher,
			String[] fields, int maxNumDocs) {
		...
		for (int docID = 0; docID < reader.maxDoc(); docID++) {
			...
			for (String field : fields) {
				 ...
				 */
				try {
					IndexableField[] multifield = reader.document(docID)
							.getFields(field);
					...
					LOGGER.warn("Document " + docID
							+ " missing requested field (" + field
							+ ")...ignoring");
				}
			}
	...
	SuggeterDataStructureBuilder(SolrIndexSearcher searcher, String[] fields,
			int ngrams, int minDocFreq, int minTermFreq, int maxNumDocs,
			String nonpruneFileName, List<String> stopWords) {
		NGRAMS = ngrams;
